...
mod softsign_integration_tests {
    use std::{
        fs,
...
impl KmsProcess {
    ...
    impl KmsProcess {
        ...
        pub fn create_tcp() -> Self {
            ...
            let args = &["start", "-c", config.path().to_str().unwrap()];
            let process = Command::new(super::KMS_EXE_PATH)
                .args(args)
                .spawn()
                .unwrap();
    ...
    fn test_handle_and_sign_vote() {
        ...
        ProtocolTester::apply(|mut pt| {
            ...
            let v_resp =
                vote::SignedVoteResponse::decode(resp.as_ref()).expect("decoding vote failed");
            let mut sign_bytes: Vec<u8> = vec![];
            svr.sign_bytes(
                chain_id.parse().unwrap(),
                ProtocolVersion::Legacy,
    ...
    fn test_handle_and_sign_get_publickey() {
        ProtocolTester::apply(|mut pt| {
            ...
            let pk_resp =
                PubKeyResponse::decode(resp.as_ref()).expect("decoding public key failed");
